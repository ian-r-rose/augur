import {
  ...
  Seed,
  hashContracts,
  writeSeeds, deployContracts, startGanacheServer
} from '..';
...
export function addGanacheScripts(flash: FlashSession) {
  ...
  flash.addScript({
    ...
    async call(this: FlashSession, args: FlashArguments) {
      ...
      await makeSeed(provider, db, config, name, true, filepath);
    },
  });
  ...
  flash.addScript({
    ...
    async call(this: FlashSession, args: FlashArguments): Promise<void> {
      ...
      await makeSeed(provider, db, this.config, name, true, seedFilePath);
    },
  });
...
async function makeSeed(provider: EthersProvider, ganacheDb: MemDown, config: SDKConfiguration, name = 'default', save = false, filepath = defaultSeedPath) {
  const seed = await createSeed(provider, ganacheDb, config.addresses);
  const warpSync = await generateWarpSyncTestData(config, seed);

  if (save) {
    const seeds = { warpSync };
    seeds[name] = seed;
